...
export const createDynamicReducer = <T extends {[x: string]: any}>(
  ...
  const {actions, reducer} = createSlice({
    ...
    reducers: {
      ...
      setQueries(state, action: PayloadAction<Record<string, string[]>>): DynamicState<T> {
        return {
          ...state,
          query: {
            ...state.query,
  ...
  const setStore = (store: Store) => _store = store;
  ...
  const getKeysByQuery = (query: string) : T | undefined => {
    return _getStore().getState()[name].query[query] || emptyArray
  }

  const sync = (item: T[]) => {
    return _getStore().dispatch(actions.multiSet(item))
  }
  ...
  return {
    ...
    useByKey, useKeysByQuery,
    getByKey, getKeysByQuery,
    setStore,
